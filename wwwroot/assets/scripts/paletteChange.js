export default (p) => {
  ...
  const palette = (typeof p === 'undefined')
    ? 0
    : pallettes[p + 1] ? p + 1 : 0;

  for (let i = 0; i < pallettes[palette].length; i++) {
    const cssVar = `--color-${i + 1}`;
    document.documentElement.style.setProperty(cssVar, pallettes[palette][i]);
  }
